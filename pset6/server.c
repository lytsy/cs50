/* The task of this problem set was to implement several methods(lookup parse load indexes). */
//
// server.c
// Computer Science 50
// Problem Set 6
...
char *indexes(const char *path)
{
    char *str = malloc(strlen(path) + 12);

    strcpy(str, path);
    strncat(str, "index.php", 9);
...
bool load(FILE *file, BYTE **content, size_t *length)
{
    size_t buffer = 32;
    BYTE b;
    BYTE *c = malloc(sizeof(BYTE) * buffer);
    BYTE *tmp;
    size_t len = 0;
    while (fread(&b, sizeof(BYTE), 1, file))
    {
        ...
        {
            buffer *= 2;
            tmp = realloc(c, sizeof(BYTE) * buffer);
            if (tmp == NULL)
            {
                return false;
            }
...
const char *lookup(const char *path)
{
    const int types_counter = 8;
    static const char *ext[types_counter] = {
        ".css",
        ".html",
        ".gif",
        ".ico",
        ...
        ".php",
        ".png"};
    static const char *type[types_counter] = {
        "text/css",
        "text/html",
        "image/gif",
        "image/x-icon",
        ...
        "text/x-php",
        "image/png"};
    char path_lower[LimitRequestLine + 1];
    memset(path_lower, 0, strlen(path_lower));
    for (int i = 0; path[i]; i++)
    {
        path_lower[i] = tolower(path[i]);
    }
    path_lower[strlen(path)] = 0;
    for (int i = 0; i < types_counter; i++)
    {
        if (strstr(path_lower, ext[i]) != NULL)
        {
            return type[i];
        }
    }
...
bool parse(const char *line, char *abs_path, char *query)
{
    ...
    strncpy(method, line, sp[0]);
    method[sp[0]] = 0;
    strncpy(request_target, line + sp[0] + 1, sp[1] - sp[0] - 1);
    request_target[sp[1] - sp[0] - 1] = 0;
    strcpy(http_version, line + sp[1] + 1);
    ...
    //request-target must start from /
    if (request_target[0] != (char)47)
    {
        error(501);
        return false;
    }
    ...
    if ((strcmp(http_version, "HTTP/1.1\r\n") != 0) && (strcmp(http_version, "HTTP/1.0\r\n") != 0))
    {
        error(505);
        return false;
    }

    char *q_start = strchr(request_target, '?');
    if (q_start == NULL)
    {
    ...
    {
        strncpy(abs_path, request_target, q_start - request_target);
        abs_path[q_start - request_target] = 0;
        strcpy(query, q_start + 1);
    }
    return true;
}
